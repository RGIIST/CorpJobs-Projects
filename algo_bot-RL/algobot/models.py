...
class DRLAgent:
    ...
    def DRL_prediction_load_from_file(model_name, environment, cwd, deterministic=True):
        ...
        try:
            ...
            print("CWD: ", cwd)
            model = MODELS[model_name].load(cwd)  ## TODO
            print("Successfully loaded model", cwd)
        except BaseException as error:
            raise ValueError(f"Failed to load agent. Error: {str(error)}") from error

        # test on the testing env
        state = environment.reset()[0].tolist()
        ...
        print("Test Finished!")
        # with open("untrnd_results.pickle", 'wb') as f:
        #     pickle.dump({"actions": actions, "episode_total_assets": episode_total_assets,
        #                  "rewards": rewards}, f)
